...
LegendreInterp1d(const LegendreInterp1d& LegendreInterp1d)
{
	initialize(LegendreInterp1d);
}

LegendreInterp1d(int interpolationOrder)
{
	initialize(interpolationOrder);
}


void initialize(const LegendreInterp1d& LegendreInterp1d)
{
    legendreInd_X            =  LegendreInterp1d.legendreInd_X;
	interpMatrixInvTranspose =  LegendreInterp1d.interpMatrixInvTranspose;
}
...
double evaluateInterpolant(double xPos, const SCC::GridFunction1d& f)
{
    ...
    {
    interpolationValue += f(i)*weightVector[weightIndex];
    weightIndex++;
    }
...
void createNodesAndWeightsData(double xPos, const SCC::GridFunction1d& f)
{
    ...
    {
    	evaluationVector[prodIndex] =legendre.evaluate(xEvalPos,pxInd);
    	prodIndex++;
    }
      ...
      SCC::DoubleVector1d& interpWeights)
{
	  ...
	  // Pack the weights into a 1d array

	  interpWeights.initialize(legendreInd_X+1);

	  long weightIndex = 0;
      for(long i = 0; i <= legendreInd_X; i++)
      {
	    interpWeights(i) = weightVector[weightIndex];
	    weightIndex++;
	  }
}
...
void createInterpolationMatrixInvTranspose()
{
    ...
    for(pxInd = 0; pxInd <= legendreInd_X; pxInd++)
    {
    interpMatrix(interpMatrixIndex,prodIndex) = legendre.evaluate(xPos,pxInd);
    prodIndex++;
    }
